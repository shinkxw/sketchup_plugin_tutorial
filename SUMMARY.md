# Summary

* [前言](README.md)

## SU插件开发基础
* [Ruby是根基](develop_base/ruby.md)
* [开发环境](develop_base/environment.md)
* [插件存放与调试](develop_base/debug.md)

## 基础库解读
* [像开发网站一样开发插件](library/develop_plugin_like_website.md)
* [调用方式与gem](library/transfer_and_gem.md)
* [本地数据存储](library/data_store.md)
* [api服务搭建](library/api_server.md)

## 演示插件开发流程
* [开发怎样的插件](demo_plugin/what_kind_of_plugin_to_develop.md)
* [插件雏形](demo_plugin/plugin_prototype.md)
* [前端构建](demo_plugin/front_part.md)
* [数据结构设计与设置](demo_plugin/data_structure_design_and_setup.md)
* [命名分层功能实现](demo_plugin/quick_naming_achieve.md)

